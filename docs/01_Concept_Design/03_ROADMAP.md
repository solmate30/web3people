...
## Next — Phase 4A: 탐색 개선 (2026-05-07 ~ 2026-05-10)

**목표**: 독자가 콘텐츠를 더 빠르게 찾을 수 있게 만든다.
...
1. 검색 기반 콘텐츠 탐색
   ...
   - 빈 결과 UI와 추천 태그
2. 인터뷰/인물 태그 체계 정리
   - 직군, 분야, 생태계, 지역/언어, 관심사 기준 정리
   - Payload `Tags` 운영 규칙 확정
3. 태그/카테고리 필터 페이지
   - `/tags/[slug]` 또는 검색 페이지 내 태그 필터 방식 결정

---

## Next — Phase 4B: 독자 참여 기반 (2026-05-11 ~ 2026-05-17)
...
## Next — Phase 4C: 성장/운영 보강 (2026-05-18 ~)

**목표**: 반복 방문과 운영 효율을 높인다.

1. 뉴스레터 구독 (이메일 수집)
2. Payload webhook -> `revalidatePath()` 즉시 캐시 갱신
3. 소셜 공유 버튼 최적화
4. 영문 병행 (선택)
