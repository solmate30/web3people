...
V2-03은 `/tags/[slug]` 독립 페이지로 구현한다. `slug`는 공개 URL의 고정 식별자가 된다. 이미 공개된 태그 slug를 바꿀 때는 리다이렉트 또는 검색 링크 영향도를 함께 확인한다.

태그 페이지는 연결된 공개 인터뷰와 공개 인물을 같은 화면에서 구분해 보여주며, 태그별 콘텐츠 총합과 타입별 개수를 표시한다.
