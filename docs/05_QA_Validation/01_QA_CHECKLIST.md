...
| # | 시나리오 | 기대 결과 | 상태 |
|:---|:---|:---|:---:|
| F-01 | 로그인 사용자가 독립 게시글 작성 | `/board` 목록과 상세에서 표시 | 구현 |
| F-02 | 인터뷰 연결 게시글 작성 | 해당 인터뷰 맥락에서 조회 가능 | 구현 |
| F-03 | 인물 연결 게시글 작성 | 해당 인물 맥락에서 조회 가능 | 구현 |
| F-04 | 비로그인 사용자가 게시글 작성 시도 | 로그인 CTA 표시, 작성 API 차단 | 구현 |
| F-05 | 작성자 본인이 게시글 수정/삭제 | 정상 처리 | 구현 |
| F-06 | 신뢰되지 않은 Origin에서 게시글 쓰기 요청 | 403으로 차단 | 구현 |
